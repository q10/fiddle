...
def growscesc(nn,idx,residue, withhydrogens):
    ...
    if resname is 'ALA':
        ...
        '''
        pdbatm2(nn,idx, 1, 1.51, 107.8,  cf(1), 0) # CG
        pdbatm2(nn,idx, 2, 1.22, 122.5,  cf(2), 0) # OD1
        pdbatm2(nn,idx, 3, 1.34, 112.7, 124.0, 1) # ND2
        '''
        if withhydrogens:
            ...
            '''
            pdbatm2(nn,idx, 4, 1.11, 109.4, 107.9, 1) # HB2
            pdbatm2(nn,idx, 5, 1.11, 109.4, 107.9, -1) # HB3
            pdbatm2(nn,idx, 6, 1.02, 119.0,  0.0, 0) # 1HD2
            pdbatm2(nn,idx, 7, 1.02, 119.0, 120.0, 1) # 2HD2
            '''

    elif resname is 'ASP' or resname is 'ASH':  # protonated ASN
        ...
        '''
        pdbatm2(nn,idx, 1, 1.51, 107.8,  cf(1), 0) # CG
        pdbatm2(nn,idx, 2, 1.25, 117.0,  cf(2), 0) # OD1
        pdbatm2(nn,idx, 3, 1.25, 117.0, 126.0, 1) # OD2
        '''
        if withhydrogens:
    ...
    '''
    elif resname is 'ASH':  # protonated ASN
        [CG, OD1, OD2] = atoms['CG'], atoms['OD1'], atoms['OD2']
        pdbatm2(CG, CB, CA, N, 1.51, 107.8,  cf(1), 0) # CG
        pdbatm2(OD1, CG, CB, CA, 1.25, 117.0,  cf(2), 0) # OD1
        pdbatm2(OD2, CG, CB, OD1, 1.25, 117.0, 126.0, 1) # OD2
        ...
        if withhydrogens:
            [HB2, HB3, HD2] = atoms['HB2'], atoms['HB3'], atoms['HD2']
            pdbatm2(HB2, CB, CA, CG, 1.11, 109.4, 107.9, 1) # HB2
            pdbatm2(HB3, CB, CA, CG, 1.11, 109.4, 107.9, -1) # HB3
            pdbatm2(HD2, OD2, CG, CB, 0.96, 109.5, 180.0, 0) # HD2


            pdbatm2(nn,idx, 4, 1.11, 109.4, 107.9, 1) # HB2
            pdbatm2(nn,idx, 5, 1.11, 109.4, 107.9, -1) # HB3
            pdbatm2(nn,idx, 6, 0.96, 109.5, 180.0, 0) # HD2

    '''

    elif resname is 'CYS':
        pdbatm2(nn,idx,1, 1.82, 109.0,  cf(1), 0) # SG
        if withhydrogens:
            pdbatm2(nn,idx, 6, 1.11, 109.4, 112.0, 1) # HB2
            pdbatm2(nn,idx, 7, 1.11, 109.4, 112.0, -1) # HB3
            pdbatm2(nn,idx, 8, 1.34, 96.0, 180.0, 0) # HG
        ...
        if withhydrogens:
            pdbatm2(nn,idx, 6, 1.11, 109.4, 109.4, 1) # HB2
            pdbatm2(nn,idx, 7, 1.11, 109.4, 109.4, -1) # HB3
            pdbatm2(nn,idx, 8, 1.10, 126.0, 126.0, 1) # HD2
            pdbatm2(nn,idx, 9, 1.10, 126.0, 126.0, 1) # HE1
            pdbatm2(nn,idx,10, 1.02, 126.0, 126.0, 1) # HE2
